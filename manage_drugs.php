...
try{
    ...
    $adminWorker = QueryFactory::getQueryFactory($_SESSION['user_id'], ", $orderByAdminQuerry", '', $conn);
    ...
    if (isset($_POST['search_1'])) {
        ...
        $Actstr = "Администратор установил строку поиска '$search_query' для таблицы лекарств.";
        $dbExecuter->insertAction($_SESSION['user_id'], $Actstr);
        $_SESSION['yummy_adm'] = $search_query;
    }
    if (isset($_SESSION['yummy_adm'])){
        $search_query_adm = $_SESSION['yummy_adm'];
        $orderByAdminQuerry = $order_by . " " . $order_dir;
        $adminWorker = QueryFactory::getQueryFactory($_SESSION['user_id'], ", $orderByAdminQuerry", "$search_query_adm", $conn);
        $result = $adminWorker->createMedicineQuery();  
    }
